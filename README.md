...
기본값은 **국판 A5 · 나눔명조 · 11pt · 줄간격 175% · 여백 18mm**입니다. 우측 하단 설정 패널의 **종이책 추천 설정**에서 에세이·수필, 소설·인문, 작은 에세이·시집, 실용서·교재, 큰 글씨 중 하나를 선택하면 판형·서체·크기·행간·여백이 함께 적용됩니다.

판형과 여백은 mm, 본문 글자 크기는 pt로 조정합니다. 본명조 계열 Noto Serif KR, 본고딕 계열 Noto Sans KR도 선택할 수 있습니다. [조사 근거, 설정표와 인쇄 시 유의점](docs/print-typography.md)을 참고하세요. 화면은 종이 전체가 보이도록 축소되며 인쇄용 PDF를 생성하는 기능은 아닙니다.

## 편집한 책을 저장소에 반영하기

Edit에서 **원문 편집**을 누르면 본문을 직접 클릭해 수정할 수 있습니다. 제목, 문단, 인용문, 박스 제목과 본문, 이미지 캡션이 각각 편집 영역이 됩니다. 별도의 원문 텍스트박스는 열리지 않습니다. **편집 마치기**로 문서 모습을 확인하고, **적용하기**로 책에 반영한 뒤 diff를 복사하거나 저장하세요. 기존 Markdown/MDX 구조와 이미지 주소는 유지됩니다.
